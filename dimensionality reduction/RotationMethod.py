...
class Rotator(object):
    ...
    def __init__(self, method='varimax', normalize=True, power=4, kappa=0, gamma=0, max_iter=500, tol=1e-5):
        self.method = method
    ...
    def _promax(self, loadings):
        ...
        try:
            diag_inv = sp.diag(np.linalg.inv(sp.dot(coef.T, coef)))
        except np.linalg.LinAlgError:
            diag_inv = sp.diag(np.linalg.pinv(sp.dot(coef.T, coef)))
    ...
    def fit_transform(self, X, y=None):
        ...
        (self.loadings_, self.rotation_, self.phi_) = new_loadings, new_rotation_mtx, phi
