...
class Kmeans (object):
    ...
    def _extendedJaccardSim(self, V1, V2):
        similarity = np.sum(V1*V2,axis=1)/(np.log(np.exp(sum(np.power(V1.T,2))) * np.exp(sum(np.power(V2.T,2))))-np.sum(V1*V2,axis=1))
        distance = 1/(1+similarity)
        return distance
        
    def _pearsonSim(self, V1, V2):
        m,n = V1.shape
        similarity = np.zeros(m)        
        if (n<3): return("pearson is not an appropriate similarity metric, please use others")
        for v1,v2,i in zip(V1, V2,range(m)):
            # Pearson correlation ranges from -1 to +1, so we normalize it to the range of 0 to 1.0.
            similarity[i] = 0.5 + 0.5*np.corrcoef(v1, v2)[0][1]            
        distance = 1/(1+similarity)
        return distance
    ...
    def fit(self, X):
        ...
        distName = ["euclidean", "cosine", "jaccard", "pearson"]
        distFun = [self._euclideanDist, self._cosineSim, self._extendedJaccardSim, self._pearsonSim]        
                
        if self.method not in distName:
            raise Exception("There are only four types of distance: euclidean, cosine, jaccard and pearson.")
